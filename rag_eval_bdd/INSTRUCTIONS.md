...
- Metric tags:
  ...
  - `@faithfulness`
  - `@completeness`
- `@live`: generates a small runtime dataset from the latest document uploaded in app UI

### Runtime live dataset generation
...
- Override count with `RAG_EVAL_LIVE_QUESTIONS_PER_LAYER=<N>`.
- Optional fallback file-path mode: set `RAG_EVAL_LIVE_DOCUMENT` and use env upload step.
- Generated files:
